...
num = (len/3)/4; % number of temporal frequencies used

rec_old = rec;
[stim_times_start, stim_times_end, rec, delete] = findTimesCorrected_new(Ch1.times, Ch1.values, -0.02, rec);
% [stim_times_start, stim_times_end] = findTimesCorrected(Ch1.times, Ch1.values, -0.02, rep, 5, 10, len);

len = len-length(delete);
...
for i = 1 : unitnbr
    [background.sum.(['unit0',num2str(i)]), background.raw.(['unit0',num2str(i)]), spikes.(['unit0',num2str(i)]), exclude.(['unit0',num2str(i)])] = testData(stim_times_start, stim_times_end, eval(['Ch',num2str(i+firstunit)]), rep);
...
xall = [.5 1 5 10 20 40 60 80 100 120];
x = x_t_p2_plot(:,1);
[~,pos] = intersect(xall,x);
x2 = [5.6 11.2 56.2 112.5 225 450 675 900 1125 1350]; % calcVelocity(x,2);
x4 = [8.4 16.8 84.3 168.7 337.5 675 1012.5 1350 1687.5 2025];
x8 = [14.1 28.1 140.6 281.2 562.5 1125 1687 2250 2812];

figure('Name','btf','NumberTitle','off'); hold on % btf
plotFreqTemp(x2, y_t_p2_plot,[1/255 102/255 94/255],'x:',y_t_p2_sd,background.sum);
plotFreqTemp(x4, y_t_p4_plot,[1/255 102/255 94/255],'x--',y_t_p4_sd,background.sum);
plotFreqTemp(x8, y_t_p8_plot,[1/255 102/255 94/255],'x-',y_t_p8_sd,background.sum);
% createLegend({'bw2','bw4','bw8'},[1/255 102/255 94/255; 1/255 102/255 94/255; 1/255 102/255 94/255],{':','--','-'},'best',1.5)
% ylim([0 450])
set(gcf,'position',[1200 500 400 350])
axis square
% print([savename,'_btf'],'-depsc','-r300','-tiff','-painters')
% savefig([savename,'_btf.fig'])

figure('Name','ftb','NumberTitle','off'); hold on % ftb
plotFreqTemp(x2, y_t_n2_plot,[140/255 81/255 10/255],'x:',y_t_n2_sd,background.sum);
plotFreqTemp(x4, y_t_n4_plot,[140/255 81/255 10/255],'x--',y_t_n4_sd,background.sum);
plotFreqTemp(x8, y_t_n8_plot,[140/255 81/255 10/255],'x-',y_t_n8_sd,background.sum);
...
% plotFreqTemp(x2(pos), y_r_p2_plot,[1/255 102/255 94/255],'x:',y_r_p2_sd,background.sum);
% plotFreqTemp(x4(pos), y_r_p4_plot,[1/255 102/255 94/255],'x--',y_r_p4_sd,background.sum);
% plotFreqTemp(x8(pos), y_r_p8_plot,[1/255 102/255 94/255],'x-',y_r_p8_sd,background.sum);
% % createLegend({'cw2','cw4','cw8'},[1/255 102/255 94/255; 1/255 102/255 94/255; 1/255 102/255 94/255],{':','--','-'},'best',1.5)
% % ylim([0 450])
% set(gcf,'position',[1200 200 400 350])
% axis square
% print([savename,'_cw'],'-depsc','-r300','-tiff','-painters')
% savefig([savename,'_cw.fig'])
% 
% figure('Name','ccw','NumberTitle','off'); hold on % ccw
% plotFreqTemp(x2(pos), y_r_n2_plot,[140/255 81/255 10/255],'x:',y_r_n2_sd,background.sum);
% plotFreqTemp(x4(pos), y_r_n4_plot,[140/255 81/255 10/255],'x--',y_r_n4_sd,background.sum);
% plotFreqTemp(x8(pos), y_r_n8_plot,[140/255 81/255 10/255],'x-',y_r_n8_sd,background.sum);
% % createLegend({'ccw2','ccw4','ccw8'},[140/255 81/255 10/255; 140/255 81/255 10/255; 140/255 81/255 10/255],{':','--','-'},'best',1.5)
% % ylim([0 450])
% set(gcf,'position',[700 200 400 350])
% axis square
